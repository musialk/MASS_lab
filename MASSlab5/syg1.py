...
sygnal1808 = pd.read_csv('1808.csv', sep=";")

#zobrazowanie danych pomiarowych
plt.figure(figsize=(12,10))
plt.subplot(3, 1, 1)
plt.plot(sygnal1808['ECG'][4000:6000])
plt.title('Sygnał ECG')
plt.subplot(3, 1, 2)
plt.plot(sygnal1808['IMP'][4000:6000])
plt.title('Sygnał IMP')
plt.subplot(3, 1, 3)
plt.plot(sygnal1808['Z0'][4000:6000])
plt.title('Sygnał Z0')
plt.show()

#__________________________Sygnal EKG_______________________________
#wyznaczenie QRS
coeffs8 = pywt.wavedec(sygnal1808['ECG'][5000:6000], 'sym6', level=4)
cA2, cD4, cD3, cD2, cD1 = coeffs8
cD4 *= 0
cD3 *= 0
cD2 *= 0
cD1 *= 0
c8 = pywt.waverec(coeffs8, 'sym6')

peaks8: ndarray = (find_peaks(c8, height=50))[0]
plt.plot(c8)
plt.plot(peaks8, c8[peaks8], "*")
plt.show()

#peaks8, properties8 = find_peaks(c8, prominence=1, width=30)
#properties8['prominences'], properties8['widths']
#plt.plot(c8)
#plt.plot(peaks8, c8[peaks8], 'x')
#plt.vlines(x=peaks8, ymin=c8[peaks8] - properties8['prominences'],
#           ymax=c8[peaks8], colors='C1')
#plt.hlines(y=properties8['width_heights'], xmin=properties8['left_ips'],
#           xmax=properties8['right_ips'], color='C1')
#plt.show()
...
coeffsIMP = pywt.wavedec(sygnal1808['IMP'][5000:6000], 'sym18', level=4)
cA1, ck4, ck3, ck2, ck1 = coeffsIMP
ck4 *= 0
ck3 *= 0
ck2 *= 0
ck1 *= 0
cIMP = pywt.waverec(coeffsIMP, 'sym18')

peaksIMP: ndarray = (find_peaks(cIMP, height=50000))[0]
plt.plot(cIMP)
...
#Obliczenie THC (1/Z0)
thc = 1/sygnal1808['Z0'][5000:6000]
plt.plot(thc)
plt.show()

#wykres wszystkich danych po obróbce
...
roznicaP1 = peaksIMP[1] - peaks8[0]
roznicaP2 = peaksIMP[2] - peaks8[1]
roznicaP3 = peaksIMP[3] - peaks8[2]
roznicaP4 = peaksIMP[4] - peaks8[3]
